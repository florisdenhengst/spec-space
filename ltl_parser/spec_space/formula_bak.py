...
from spec_space.parser.lexer import BaseSymbolSet
from spec_space.attribute import Attribute
#from abc import abstractmethod
from spec_space.observer import Observer
from spec_space import LOG
...
class IntervalSet:
    ...
    ''' Add given range to this timed variable. '''
    def merge(self, range):
        for i in intervals:
            if i.continuous_union(range):
                i.merge(range)
                return
        self.intervals.add(range)

    ''' Shift each interval by one. '''
    def increment(self, max_n):
        for i in intervals:
            if i.start <= max_n:
                i.start += 1
            if i.end <= max_n:
    ...
    ''' Set interval to lowest-start-index range until max_n.'''
    def set_min_through_n(self, max_n):
        t = float(inf)
        idx = 0
...
class Range:
    ...
    def continuous_union(self, other):
        if other is None:
            return False
        if min(self.end, other.end) >= max(self.start, other.end):
            return True;
    ...
    def merge(self, other):
        if other is None:
            ...
            self.start = min(r1.start, r2.start)
            self.end = max(r1.end, r2.end);
...
''' Maps variable names to sets of intervals. '''
class VarMap:
    assigned = {};

    ''' Add a variable and corresponding interval set to this map. '''
    def add(self, var, intervals):
        if var in self.assigned:
            self.assigned[var].merge(intervals)
        else:
            self.assigned[var] = intervals



    ''' Return true if this map intersects with another. '''
    def intersects(self, other):
        if other is None:
            return False
        else:
            for var in assigned:
                if other.assigned[var] is not None:
                    for i in self.assigned[var].intervals:
                        for j in other.assigned[var].intervals:
                            if i.overlaps(j):
                                return True
        return False
        ...
        if other is None:
            ...
            for var in self.assigned:
                if other.assigned[var] is not None:
                    for i in self.assigned[var].intervals:
                        for j in other.assigned[var].intervals:
                            if self.assigned[var].intervals[i].continuous_union(other.assigned[var].intervals[j]):
                                self.assigned[var].intervals[i].merge(other.assigned[var].intervals[j])
                            else:
                                self.assigned[var].intervals.append(other.assigned[var].intervals[j])
...
class Literal(Attribute, LTLFormula):
    ...
    def update_deps(self):
        print("HERE")
        self.deps.add(self.base_name, IntervalSet(Range(0,1)))
...
class BinaryFormula(LTLFormula):
    ...
    def bound(self, n, i):
        self.left_formula.bound(n, i);
        self.right_formula.bound(n, i);

    def update_deps(self): # get timed literals
        print("HERE")
        left = self.left_formula.update_deps();
        right = self.right_formula.update_deps();
        self.deps = self.left_formula.deps.merge(self.right_formula.deps);
        return deepcopy(self.deps);
...
class UnaryFormula(LTLFormula):
    ...
    def update_deps(self):
        self.deps = self.right_formula.update_deps();
        return deepcopy(self.deps)
...
class Globally(UnaryFormula):
    ...
    def bound(self, n, i):
        for x in range(n-i):
            self.right_formula = Next(self.right_formula);
